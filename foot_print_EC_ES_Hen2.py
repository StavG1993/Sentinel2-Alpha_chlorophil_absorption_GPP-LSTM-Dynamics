# -*- coding: utf-8 -*-
"""
Created on Mon May 12 11:29:14 2025
...
#%%
df_path = r"D:\Stav\research\ES_Hen2\Daily_Es_Hen2.csv" 
...
rho = 1.2    # air density [kg/m³] (assumed)
## non changed variables ES_Hen
zm = 10.2        # Measurement height [m]
domain = [-1000, 1000, -1000, 1000]
dx = dy = 2
# Set your target UTM CRS (example: UTM zone 36N, change as needed)
crs_utm = "EPSG:32636"
# Output directory for shapefiles
output_dir = "D:/Stav/research/ES_Hen2/polygons_footprint/" 
...
#%%
d_point = {"latitude":38.41, "longitude":-1.7}
df_point = pd.DataFrame(d_point, index=[0])
gdf_point = gpd.GeoDataFrame(
df_point, geometry=gpd.points_from_xy(df_point.longitude, df_point.latitude), crs="EPSG:4326"
)
gdf_point = gdf_point.to_crs("EPSG:25830")
x_point = gdf_point.bounds["minx"][0]
y_point = gdf_point.bounds["miny"][0]
for i in range(len(df)):
    print(f"now working on row number {i+1}")
    wind_speed = df["WS"].iloc[i]
    wind_dir = df["WindDir"].iloc[i]
    date_str = df['Date'].iloc[i]
    day = date_str.split("_")[0]
    month = date_str.split("_")[1]
    year = date_str.split("_")[2]
    date_str = f"{day}_{month}_{year}"
    T_air = df['Tair'].iloc[i]
    H = df['H'].iloc[i]
    ustar=df['USTAR'].iloc[i]
    if np.any(np.isnan([wind_speed, wind_dir,T_air, H])):
        print(f"row {i+1} has none so we won't run")
        continue
    print(f"row is ok {i+1}")
    folder_out = f"{output_dir}/{date_str}"
    T_kelvin = T_air + 273.15
    # ustar = (kappa * wind_speed) / np.log((zm - d) / h0)
    ol = -(ustar**3 * T_kelvin) / (kappa * g * (H / (rho * cp)))
    sigmav = 2.0 * ustar
    h = 10 * zm
    try:
        result = FFP_climatology(
            zm=zm,
            z0=z0,
            umean=wind_speed,
            h=h,
        ...
        lon_contur = np.array(result["xr"]) + x_point
        lat_contur = np.array(result["yr"]) + y_point
        polygon_geom = Polygon(zip(lon_contur[0], lat_contur[0]))
        polygon = gpd.GeoDataFrame(index=[0], crs="EPSG:25830", geometry=[polygon_geom])
        if ~os.path.exists(folder_out): ## if the folder NOT exists
            os.makedirs(folder_out)
            print(f"created dir {folder_out}")
        else:
            print("dir exists")
        out_file_name = f"{folder_out}/{date_str}.shp"    
        polygon.to_file(out_file_name)
        print(f"row {i+1} is done")
            
    except Exception as e:
        print(f"fail to run row number {i+1}")
        print(e)
...
#merging all the footprints to 1 folder for the upload for GEE

# Directory with folders named like "26_02_2025"
import zipfile
root_dir = r"D:\Stav\research\ES_Hen2\polygons_footprint" 

features = []

for folder in os.listdir(root_dir):
    folder_path = os.path.join(root_dir, folder)
    if os.path.isdir(folder_path) and folder.count("_") == 2:
        shp_files = [f for f in os.listdir(folder_path) if f.endswith(".shp")]
        if shp_files:
            shp_path = os.path.join(folder_path, shp_files[0])
            gdf = gpd.read_file(shp_path)

            # Step 2: Assign CRS manually if missing
            if gdf.crs is None:
                print(f"CRS is missing in {folder}. Assigning EPSG:25830...")
                gdf.set_crs(epsg=25830, inplace=True)

            # Step 3: Reproject to EPSG:4326
            gdf = gdf.to_crs("EPSG:4326")

            gdf["date"] = folder  # Add date property from folder name
            features.append(gdf)


# Combine all into one GeoDataFrame
gdf_all = pd.concat(features, ignore_index=True)
gdf_all.to_file("D:/Stav/research/ES_Hen2/polygon_footprint_merged/ES_Hen2_merged.shp") 

## making it into zip fole so GEE can read them all together

# Path to the folder containing the shapefile components
shapefile_dir = r"D:\Stav\research\ES_Hen2\polygons_footprint"
shapefile_base = "ES_Hen2_merged"
output_zip = os.path.join(shapefile_dir, shapefile_base + ".zip")

# List all required components
extensions = [".shp", ".shx", ".dbf", ".prj", ".cpg"]  # .cpg optional
with zipfile.ZipFile(output_zip, "w") as zipf:
    for ext in extensions:
        file_path = os.path.join(shapefile_dir, shapefile_base + ext)
...
## converting the polygon shapefile that Ive created in QGIS to the correct coordinates

# Path to your shapefile
shp_path = "D:/Stav/research/ES_Hen2/polygon_footprint_dissolved/Es_Hen2_dissolved.shp" 
...
# Step 2: Assign CRS manually if missing
if gdf.crs is None:
    print("CRS is missing. Assigning EPSG:25830 (ETRS89 / UTM zone 30N)...")
    gdf.set_crs(epsg=25830, inplace=True)

# Step 3: Reproject to EPSG:4326 (WGS84)
gdf_wgs84 = gdf.to_crs("EPSG:4326")

# Step 4: Save reprojected shapefile
output_dir = "D:/Stav/research/ES_Hen2/polygon_footprint_dissolved"
output_base = "fp_dissolved_Hen2_wgs84"
gdf_wgs84.to_file(os.path.join(output_dir, output_base + ".shp"))

# Step 5: Zip all required files
extensions = [".shp", ".shx", ".dbf", ".prj", ".cpg"]
with zipfile.ZipFile(os.path.join(output_dir, output_base + ".zip"), 'w') as zipf:
    for ext in extensions:
        file_path = os.path.join(output_dir, output_base + ext)
        if os.path.exists(file_path):
            zipf.write(file_path, arcname=os.path.basename(file_path))
        else:
            print(f"Warning: {ext} missing!")

print("✅ Reprojected and zipped shapefile ready for GEE upload.")
